import { requestThroughGitHubActions, _setRetryIntervalForTesting } from '../github';
import { AxiosInstance } from 'axios';

describe('getProofThroughGitHubActions', () => {
  let mockClient: {
    post: jest.Mock;
  };
  ...
  it('should error out if the start request fails', async () => {
    ...
    await expect(requestThroughGitHubActions(client(), '1')).rejects.toMatchSnapshot();
  });
  ...
  it('should log out the provided proof content', async () => {
    ...
    await requestThroughGitHubActions(client(), '1');
  ...
  it('should keep trying to acquire until content is available', async () => {
    ...
    await expect(requestThroughGitHubActions(client(), '1')).resolves.toEqual({ token: '12345' });
  });
});
